...
void UIText::OnMeasure(UIWindow*destWindow) {
	...
	if (textLayout) {
		...
		else textLayout->SetWordWrapping(DWRITE_WORD_WRAPPING_WRAP);

		if (textOverFlowHidden) {
			IDWriteInlineObject *obj = 0;
			UIElement::dwriteFactory->CreateEllipsisTrimmingSign(textLayout, &obj);
			textLayout->SetTrimming(&trimming,obj);
		}
