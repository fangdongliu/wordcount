...
void UIInput::OnMouse(MouseEvent&e) {
	static Timer timer;
	float lastTime=timer.TotalTime();
	if (!e.GetEventHandledId())
		if (e.GetDestWindow()->GetCaptureElement())
			if (e.GetDestWindow()->GetCaptureElement() != this)
				return;
	...
	case MouseEventMButtonDown:
		if (e.GetEventHandledId() != 2) {
			if (!e.GetEventHandledId()) {
				e.GetDestWindow()->SetCaptureElement(this);
				e.GetDestWindow()->SetHoverElement(this);
			}
			if (isFocusable) {
				e.GetDestWindow()->SetFocusElement(this);
				e.EventHandled(2);
			}
			else {
				e.EventHandled(1);
	...
	case MouseEventMouseMove:
		...
		else if (e.GetEventHandledId() == 1)
			ChangeCurrentStyle(e.GetDestWindow(), UIState::UIStateHover);
	...
	{
		timer.Tick();
		if (e.GetDestWindow()->lastClickElement == this && timer.TotalTime() - lastTime < 0.3f) {
			caretAnchor = 0;
			caretPosition = contentText.length();
			caretPositionOffset = 0;
			...
			e.GetDestWindow()->lastClickElement = this;
			DWRITE_HIT_TEST_METRICS metrics = { 0 };
			BOOL isTrailing;
			BOOL isInside;
			textLayout->HitTestPoint(e.GetPoint().X - rect.X - currentStyle.padding.Left, e.GetPoint().Y - rect.Y - currentStyle.padding.Top, &isTrailing, &isInside, &metrics);
			caretPosition = metrics.textPosition;
			if (isTrailing) {
		...
		D2D1_RECT_F caretRect;
		GetCaretRect(caretRect);
		UpdateSystemCaret(e.GetDestWindow(), caretRect);		
	}
		break;
	...
	if (mouseEventListener) {
		e.SetDestElement(this);
		mouseEventListener->OnMouse(e);
	}
