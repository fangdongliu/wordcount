...
void LinesPainter::SetLines(int count, ...) {
	...
	for (int i = 0; i < count; i++) {
		lineList.push_back({ (D2D1_POINT_2F)va_arg(ap,D2D1_POINT_2F),(D2D1_POINT_2F)va_arg(ap,D2D1_POINT_2F) });
	}
}
...
UIElement* Page::CreateCloseBtn() {
	...
	ColorF color(0x888888, 0.7);
	hoverStyle->SetBackgroundColor(color);
...
UIRelativeContainer* Page::CreateDraggableBackground() {
	...
	relative->mouseEventListener = new NoParamMouseListener{
		[](MouseEvent&e)->void {
		if (e.GetEventType() == MouseEventType::MouseEventLButtonDown && e.GetEventHandledId()!=2) {
			ReleaseCapture();
			e.EventHandled(2);
