...
namespace {
	static unordered_set<wstring>supports = {
		L"cpp",L"c",L"h",L"hpp",L"cxx",L"cs",L"jsp",L"java",L"js"
	};
	App *app;
	UIRelativeContainer* CreateHead() {
		...
		auto icon = new UIElement;
		{
			...
			style.SetSizeCaculator(size);
			wstring ic = L"home.png";
			style.SetBackgroundImage(ic);
			style.SetLeft(10.0f);
			style.SetTop(5.0f);
		}
		...
		auto path = new UIInput;
		{
			...
			SizeCaculator size(400.0f, 30.0f, 0, 0, 0);
			style.SetSizeCaculator(size);
			path->contentText = L"E:\\源代码\\Windows-classic-samples-master\\Samples\\Win7Samples\\multimedia\\DirectWrite\\CustomLayout";
		...
		auto pathWrapper = new UIRelativeContainer;
		{
			...
			SizeCaculator size(450.0f, 30.0f, 0, 0, 0);
			style.SetSizeCaculator(size);
			style.SetBorderRadius(15.0f);
			style.SetLeft(150.0f);
			UIBorder border = { D2D1::ColorF(D2D1::ColorF::Black),1.0f,1,1,1,1 };
			style.SetBorder(border);

			D2D1::ColorF white(D2D1::ColorF::White, 0.5);
		...
		//	style->SetDuration(0.3f);
			...
			style = new UIStyle;
			style->SetSizeCaculator(size);
			ColorF color2(ColorF::LightBlue, 0.0f);
			style->SetBackgroundColor(color2);
			style->SetRight(50.0f);
			style->SetTop(0);
		//	style->SetDuration(0.3f);
...
App::App() {
	...
	pathStack.push(path);
	SHFILEINFOW info;
	SHGetFileInfoW(L"C:\\Users\\fangdongliu\\source\\repos\\wc\\wcGUI\\app.cpp", 0,&info ,sizeof(info), SHGFI_ICON);
	InitFDUI(info.hIcon);
...
void App::MouseListener::OnMouse(MouseEvent&e) {
	...
	case MouseEventType::MouseEventLButtonDown:
		...
		if(isFolder){
			if (lastClickObj == e.GetDestElement() && timer.TotalTime() - lastTime < 0.3f) {
				lastClickObj = 0;
				e.EventHandled(2);
				WCHAR path[1000];
...
void App::ResourceIniter::OnDeviceResourceChange() {
	...
	D2D1_GRADIENT_STOP stop[] = {
		{0.0f,ColorF(0x92a8d1)},{1.0f,ColorF(0xf7cac9)}
	};
