...
void UIElement::ChangeCurrentStyle(UIWindow*destWindow, UIState state_,float duration_) {
	...
	if (destWindow->GetFocusElement() == this&&state_==UIStateNormal)
		return;
...
void UIElement::OnPaint(UIWindow *destWindow) {
	...
	if (useLayer) {
		...
		destWindow->currentRenderTarget->PushAxisAlignedClip(D2D1::RectF(rect.X, rect.Y, rect.X + rect.Width, rect.Y + rect.Height), D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
	}
	...
	if (useLayer)
		...
		destWindow->currentRenderTarget->PopAxisAlignedClip();
	if (transformValid)
		destWindow->currentRenderTarget->SetTransform(lastTransform);
}
...
void UIElement::OnMouse(MouseEvent &mouseEvent) {
	...
	if(!mouseEvent.GetEventHandledId())
		if (mouseEvent.GetDestWindow()->GetCaptureElement())
			if (mouseEvent.GetDestWindow()->GetCaptureElement() != this) 
				return;
	...
	switch (mouseEvent.GetEventType()) {
		...
		case MouseEventMButtonDown:
			if (mouseEvent.GetEventHandledId()!=2) {
				if (!mouseEvent.GetEventHandledId()) {
				...
				if (isFocusable) {
					...
					mouseEvent.EventHandled(2);
				}
				else {
					mouseEvent.EventHandled(1);
				}
		...
		case MouseEventMouseMove:
			if (!mouseEvent.GetEventHandledId()) {
				...
				else mouseEvent.EventHandled();
			}
			else if(mouseEvent.GetEventHandledId()==1)
...
void UIElement::PaintBackground(UIWindow*window) {
	...
	if (currentStyle.IsFlagValid(AttributionFlag_background)) {
		...
		if (currentStyle.background.backgroundColor.a != 0.0f) {
		...
		if (image) {
			...
			if (currentStyle.IsFlagValid(AttributionFlag_borderRadius))
				window->currentRenderTarget->FillRoundedRectangle(
					D2D1::RoundedRect(rectangle, currentStyle.borderRadius, currentStyle.borderRadius), window->bitmapBrush);
			else
				window->currentRenderTarget->FillRectangle(rectangle, window->bitmapBrush);
		}
...
void UIElement::PaintBorder(UIWindow*window) {
	...
	if (currentStyle.IsFlagValid(AttributionFlag_border)) {
		auto offset = currentStyle.border.borderSize / 2;
		auto rectangle = D2D1::RectF(rect.X+ offset, rect.Y+ offset, rect.X + rect.Width - offset, rect.Y + rect.Height - offset);
